...
{
    class CardEffectEmberwaveEmberDmg : CardEffectBase
    {
          ...
          CardEffectParams cardEffectParams)
        {
            ...
            for (int i = 0; i < cardEffectParams.targets.Count; ++i)
            {
                ...
                for (int j = 0; j < cardEffectState.GetParamInt(); j++)
                {
                    yield return (object)cardEffectParams.combatManager.ApplyDamageToTarget(cardEffectParams.playerManager.GetEnergy(), target, new CombatManager.ApplyDamageToTargetParameters()
                    {
